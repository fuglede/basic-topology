...
\end{example}
\begin{example}
  Let $I \subset \bbR$ be any interval (bounded, unbounded, open, closed, or half-open). We claim that $I$ is connected. To see this, assume that $I = U \cup V$ is a separation of $I$, let $x \in U$, $y \in V$, and assume without loss of generality that $x < y$. Then we have $[x,y] \subset I$. Let $U_0 = [x,y] \cap U$, $V_0 = [x,y] \cap V$ so that $[x,y] = U_0 \cap V_0$ is a separation of $[x,y]$, and define $z = \sup U_0 \in [x,y]$. We will show that $z$ belongs to neither $U_0$ or $V_0$ which is a contradiction.
  
  Suppose first that $z \in U_0$. Then $z \not= y$ so either $z = x$ or $x < z < y$. In any case we can choose a $u$ so that $z < u \leq y$ and $[z,u) \subset U_0$ since $U_0$ is open in $[x,y]$.
\end{example}
\begin{lem}
...
\end{proof}
\begin{example}
  We can now show that intervals in $\bbR$ are connected.
\end{example}
