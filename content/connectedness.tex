...
\label{connectedness}
In $\bbR^n$, we have a good intuition about what it means for subsets to be connected or not. For example, the subset $(-2,-1) \cup (1,2)$ does seem very connected: how would we connect $-1$ and $1$? On the other hand, $(-2,2)$ should probably deserve to be called connected.
...
\begin{proof}
  ...
  Similarly, fix $x \in C_i$. Then for every other $y \in C_i$ there a connected subset $A_y$ so that $x, y \in A_y$. Now we use our usual trick and find that $C_i = \bigcup_{y \in C_i} A_y$. Since all of the $A_y$ contain $x$, we use Theorem~\ref{unions-connected} to conclude that $C_i$ is connected.
...
\begin{lem}
  The relation $\sim_{\mathrm{path}}$ is an equivalence relation. The equivalence classes of $\sim_{\mathrm{path}}$ are called the \word{path-connected components}{b{\aa}gvis sammanh{\"a}ngande komponenter}{path-connected component}{b{\aa}gvis sammanh{\"a}ngande komponent} of $X$.
\end{lem}
