...
\begin{defn}
  ...
  A topological space $X$ is called \word{locally Euclidean}{lokalt euklidisk} if there is an $n \in \bbN$ so that every point in $X$ has a neighbourhood which is homeomorphic to $\bbR^n$.
\end{defn}
\trans{locally Euclidean}{lokalt euklidisk}
\begin{defn}
...
\end{thm}
\begin{defn}
  Let $X$ be a topological space and $f : X \to \bbR$ a function. The \word{support}{st{\"o}d} of $f$ is the set
  \[
...
\trans{support}{st{\"o}d}
\begin{defn}
  Let $X$ be a topological space, and let $\{U_1, \dots, U_n\}$ be an open cover of $X$. A family $\{\phi_1,\dots,\phi_n\}$ of continuous functions $\phi_i : X \to [0,1]$ is called a \word{partition of unity}{partition av enheten} dominated by $\{U_i\}$ if
  \begin{itemize}
    \item $\supp(\phi_i) \subset U_i$ for $i = 1, \dots, n$, and
    \item $\sum_{i=1}^n \phi_i(x) = 1$ for all $x \in X$.
  \end{itemize}
\end{defn}
\trans{partition of unity}{partition av enheten}
\begin{thm}
...
\begin{defn}
  Let $X$ be a topological space. A collection $\calU$ of subsets of $X$ is called \word{locally finite}{lokalt {\"a}ndlig} if every point of $X$ has a neighbourhood that intersects only finitely many elements of $\calU$.
\end{defn}
\trans{locally finite}{lokalt {\"a}ndlig}
\begin{defn}
  A topological space $X$ is called \word{paracompact}{parakompakt} if every open cover has a locally finite subcover.
\end{defn}
\trans{paracompact}{parakompakt}
...
\begin{thm}
  ...
  \begin{itemize}
    ...
    \item[(iii)] $\sum_{i \in I} \phi_i(x) = 1$ for every $x \in X$.
  \end{itemize}
\end{thm}
