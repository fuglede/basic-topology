...
\end{proof}
In analysis, one might have encountered the important characterization of continuity that for sequences in $\bbR^n$, $x_n \to x$ if and only $f(x_n) \to f(x)$ when $f$ is continuous. It turns out that this is not true for general continuous maps between topological spaces without further assumptions.
\begin{defn}
