...
\end{example}
\begin{example}
  All metric spaces (with the metric topology) are Hausdorff: If $x$ and $y$ are distinct points in a metric space $(X,d)$, let $r = d(x,y)/2$, and define $U_x = B_d(x,r)$, $U_y = B_d(y,r)$. Then $U_x$ and $U_y$ are distinct neighbourhoods of $x$ and $y$ respectively.
\end{example}
