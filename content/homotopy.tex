...
If $G$ and $H$ are two groups, then their Cartesian product $G \times H$ is a group with multiplication given by
\[
...
We now turn to the proofs of Lemmas~\ref{path-lifting-lemma} and \ref{homotopy-lifting-lemma} where will need the following technical lemma.
\begin{lem}[Lebesgue number lemma]
  Let $\calU$ be an open cover of a compact metric space $(X,d)$. Then there exists a $\delta > $
\end{lem}
