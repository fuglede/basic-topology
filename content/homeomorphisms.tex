...
\begin{defn}
  Let $X$ and $Y$ be topological spaces, and let $p : X \to Y$ be a surjective map. Then $p$ is called a \word{quotient map}{kvotavbildning} if it has the property that $U \subset Y$ is open if and only if $p^{-1}(U) \subset X$ is open.
\end{defn}
\trans{quotient map}{kvotavbildning}
...
\begin{defn}
  ...
  Let $X$ be a topological space with an equivalence relation $\sim$. Let $p : X \to X/\!\sim$ be the map $p(x) = [x]$. The \wordexp{quotient topology}{kvottopologin}{quotient topology, the}{kvottopologin} on $X/\!\sim$ is the topology defined by saying that $U \subset X/\!\sim$ is open if $p^{-1}(U) \subset X$ is open. In other words, it is the unique topology that forces $p$ to be a quotient map.
\end{defn}
\trans{quotient topology}{kvottopologin}
...
With this relation, the equivalence classes are exactly the sets in $X$ of the form $f^{-1}(\{y\})$ for $y \in Y$, and as before, there is a bijection $g: X/\!\sim_f \to Y$ given by $g(f^{-1}(\{y\})) = y$; that is, $f = g \circ p$ where $p : X \to X/\!\sim_f$ is as before. Notice that we need $f$ to be surjective for this construction to work.
\begin{prop}
