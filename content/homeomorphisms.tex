...
\begin{rem}
  ...
  If $q = (0,\dots,0,-1) \in S^n$ is the south pole, then the reflection map $(x_1, \dots, x_n,x_{n+1}) \mapsto (x_1,\dots,x_n,-x_{n+1})$ is a homeomorphism from $S^n \setminus \{p\}$ to $S^n \setminus \{ q \}$, so we also have that $S^n \setminus \{ q \}$ is homeomorphic with $\bbR^n$. More generally, one can show $S^n \setminus \{x\} \simeq \bbR^n$ for all $x \in S^n$.
\end{rem}
\begin{example}
  Let $f : [0,1) \to S^1$ be the map $f(x) = (\cos (2\pi x), \sin(2\pi x))$. Then $f$ is a bijection and moreover, $f$ is continuous by Proposition~\ref{props-subspace-top} and Lemma~\ref{universal-inclusion-lemma}: consider the map $\tilde{f} : \bbR \to \bbR^2$ given by $\tilde{f}(x) = (\cos (2\pi x), \sin(2\pi x))$. This is clearly continuous, so its restriction $\tilde{f}|_{[0,1)} : [0,1) \to \bbR^2$ is continuous. If $\iota : S^1 \to \bbR^2$ denotes the inclusion map, then $\tilde{f}|_{[0,1)} = \iota \circ f$, so $f$ is continuous.
