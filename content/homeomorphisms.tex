...
\begin{rem}
  ...
  If $q = (0,\dots,0,-1) \in S^n$ is the south pole, then the reflection map $r: (x_1, \dots, x_n,x_{n+1}) \mapsto (x_1,\dots,x_n,-x_{n+1})$ is a homeomorphism from $S^n \setminus \{p\}$ to $S^n \setminus \{ q \}$, so we also have that $S^n \setminus \{ q \}$ is homeomorphic with $\bbR^n$.
