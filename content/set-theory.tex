...
\[
  A \subsetneq B
\]
and say that $A$ is a \word{proper subset}{{\"a}kta delm{\"a}ngd} of $B$.\footnote{Notice that it is also common to use the notation $A \subseteq B$ for the relation ``$A$ is a subset of $B$''.} Set-theoretic relationships are often depicted in so-called Euler diagrams; see Figure~\ref{eulersubset}.
\begin{figure}
  \centering
