...
\begin{enumerate}[label=1.\arabic*]
  ...
  \item Let $X$ be a topological space, and let $A$ and $B$ be subspaces of $X$ so that $A \subset B \subset X$. Then $A$ can be endowed with the subspace topology $\calT_A$ from $X$, or the subspace topology $(\calT_B)_A$ from $B$ (which in turn has the subspace topology coming from $X$). Show that $\calT_A = (\calT_B)_A$.
...
\begin{enumerate}[label=2.\arabic*]
  ...
  \item A space is called \emph{totally disconnected}\index{totally disconnected} if its only non-empty connected subspaces are one-point sets. Show that if $X$ has the discrete topology, then $X$ is totally disconnected. Does the converse hold?
  \item Let $f : S^1 \to \bbR$ be continuous. Show that there is a point $x \in S^1$ so that $f(x) = f(-x)$.
  \item \label{Brouwer-interval-exercise} Let $f : [0,1] \to [0,1]$ be continuous. Show that $f$ has a fixed point, i.e. that there is a point $x \in [0,1]$ so that $f(x) = x$.
...
\subsection{Set \#3}
  \begin{enumerate}[label=3.\arabic*]
    ...
    \begin{itemize}
      \item[(a)] Show that if $K_1, \dots, K_n$ are compact subspaces of $X$, then $K_1 \cup \dots \cup K_n$ is compact.
    ...
    \end{itemize}
    \item Let $A \subset X$ be a subspace of a topological space $(X,\calT)$. Show that the following are equivalent:
    \begin{itemize}
      \item $A$ is compact (in the subspace topology)
      \item For every collection of open sets $\calU \subset \calT$ so that $A \subset \bigcup_{U \in \calU}U$, there exists a finite subcollection $U_1, \dots, U_n \in \calU$ so that $A \subset U_1 \cup \dots \cup U_n$.
    \end{itemize}
    ...
    \item \label{second-countable-exercise}Show that $\{B(x,r) \mid x \in \bbQ^n, r \in \bbQ_{> 0}\}$ is a basis, and that it generates the standard topology on $\bbR^n$. Conclude that $\bbR^n$ is second-countable.
    \item Let $Y$ be a compact space, and let $X$ be any topological space.
      ...
      \item[(b)] Suppose moreover that $Y$ is Hausdorff, and let $f : X \to Y$ be a map. Show that $f$ is continuous if and only if its graph
        ...
        is closed.
      \end{itemize}
    \item Let $(X,\preceq)$ be a totally ordered set with the order topology, and assume that every closed interval $[a,b]$ is compact. Show that $X$ has the \emph{least-upper-bound property}\index{least-upper-bound property}; that is, show that every non-empty subset of $X$ which is bounded from above has a least upper bound in $X$.
...
\begin{enumerate}[label=4.\arabic*]
  ...
  \item A subspace $A \subset \bbR^n$ is called \emph{star-shaped}\index{star-shaped} if there exists an element $a \in A$ so that for any $x \in A$, the line segment from $a$ to $x$ is contained in $A$. Show that star-shaped sets are simply-connected.
  ...
  \item \label{fundamental-group-punctured-space-exercise} Let $n \geq 2$. Show that $\bbR^n \setminus \{0\}$ is homeomorphic to $S^{n-1} \times (0,\infty)$. Is $\bbR^n \setminus \{0\}$ simply-connected?
  \item Define an equivalence relation $\sim$ on $S^n$ by saying that $x \sim y$ if $x = \pm y$. Let $\bbRP^n = S^n/\sim$ denote the quotient space, called the \word{real projective space}{reella projektiva rummet}. Show that for $n \geq 2$, we have $\pi_1(\bbRP^n) = \bbZ_2$, the (only) group containing two elements. (Hint: show that the projection map $p : S^n \to \bbRP^n$ is a covering map.)
  \item Let $A \subset X$ be a subspace of a topological space, and let $r : X \to A$ be a continuous map so that $r|_A = \id_A$ (such an $r$ is called a \emph{retraction}\index{retraction}). Let $a \in A$.
    \begin{itemize}
      \item[(a)] Show that $r_* : \pi_1(X,a) \to \pi_1(A,a)$ is surjective.
      \item[(b)] Suppose moreover that there is a homotopy $F : X \times [0,1] \to X$ from $\id_X$ to $r$ with the property that $F(x,t) = x$ for all $x \in A$ (such an $F$ is called a \emph{deformation retraction}\index{deformation retraction}). Show that $r_*$ is an isomorphism.
      ...
      This is a special case of Brouwer's fixed-point theorem\index{Brouwer's fixed-point-theorem} which says that in general, any continuous map $A \to A$, where $A \subset \bbR^n$ is compact and convex, has a fixed point. The case $A = [0,1]$ was covered in Exercise~\ref{Brouwer-interval-exercise}.\footnote{See \url{https://en.wikipedia.org/wiki/Brouwer_fixed-point_theorem\#Illustrations} for a couple of curious ``real world applications'' of this theorem.}
    \end{itemize}
\end{enumerate}
