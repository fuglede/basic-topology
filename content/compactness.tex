...
\begin{proof}
  ...
  \]
  This sequence has the property that $d(x_n,x_m) > r$ for all $n, m \in \bbN$, $n \not= m$,s so it can have no convergent subsequence (recall the characterization of convergence in metric spaces from Proposition~\ref{metric-convergence}), which is our contradiction, so there is a finite subcover $\{B_d(x_i,r)\}_{i=1,\dots,n}$.
